            ...
            for (int j = 0; j < graph.getV(); j++) {
                ...
                if (graph.getVertexState(j) == Graph.VertexState.ACTIVE) {
                    for (int k = 0; k < graph.getV(); k++) {
                        labels[j][k].setOpaque(true);
                        labels[k][j].setOpaque(true);
                        labels[j][k].setBackground(new Color(Graph.EdgeState.SELECTED.getColor()));
                        labels[k][j].setBackground(new Color(Graph.EdgeState.SELECTED.getColor()));
                    }
